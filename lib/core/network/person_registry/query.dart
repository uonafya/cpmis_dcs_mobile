  ...
  static Future<List<RegisterNewChildModel>> getRegistryFormDetails() async {
    try {
      ...
      final List<Map<String, dynamic>> maps =
          await db.query(registryFormDetails);
      ...
      List<RegisterNewChildModel> list = List.generate(maps.length, (i) {
        return RegisterNewChildModel(
          ...
          registryIdentificationModel: RegistryIdentificationModel.fromJson(
              jsonDecode(maps[i]['registryIdentificationModel'])),
          registryContactDetailsModel: RegistryContactDetailsModel.fromJson(
              jsonDecode(maps[i]['registryContactDetailsModel'])),
          registryLocationModel: RegistryLocationModel.fromJson(
              jsonDecode(maps[i]['registryLocationModel'])),
          caregivers: (jsonDecode(maps[i]['caregivers']) as List)
              .map((c) => RegistryCaregiverModel.fromJson(c))
              .toList(),
          siblings: (jsonDecode(maps[i]['siblings']) as List)
              .map((s) => RegistrySiblingModel.fromJson(s))
              .toList(),
          registryCboChvModel: RegistryCboChvModel.fromJson(
              jsonDecode(maps[i]['registryCboChvModel'])),
