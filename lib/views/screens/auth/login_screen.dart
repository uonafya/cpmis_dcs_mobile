...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _authenticate() async {
    ...
    if (username != null && password != null) {
      _login(username: username, password: password, isBiometerics: true);
    } else {
      if (context.mounted) {
  ...
  void _login({
    ...
    bool isBiometerics = false,
  }) async {
    if (!isBiometerics && !formKey.currentState!.validate()) {
      return;
    }
    setState(() {
      _isloading = true;
    ...
    try {
      ...
      showErrorSnackBar(context, e.error.toString());
      setState(() {
        _isloading = false;
      });
    }
